...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        //        lifeLabel.position = CGPoint(x: self.size.width * 0.85, y: self.size.height + lifeLabel.frame.size.height)
        //        lifeLabel.zPosition = 101
        //        self.addChild(lifeLabel)
        
        
        let moveOnToScreenAction = SKAction.moveTo(y: self.size.height*0.9, duration: 0.3)
        scorelabel.run(moveOnToScreenAction)
        //        lifeLabel.run(moveOnToScreenAction)
    ...
    func spawnExplosion(spawnPos: CGPoint) {
        ...
        //let explosion = SKSpriteNode(imageNamed: "spark.png")
    ...
    //    func loseLife () {
    //        lifeScore -= 1
    //        lifeLabel.text = "Lives: \(lifeScore)"
    //        let scaleUp = SKAction.scale(to: 1.5, duration: 0.2)
    //        let scaleDown = SKAction.scale(to: 1, duration: 0.2)
    //        let scaleSequence = SKAction.sequence([scaleUp, scaleDown])
